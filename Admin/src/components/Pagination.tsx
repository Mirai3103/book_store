...
const createClass = (current: number, index: number | string) => {
    ...
    return "px-3 py-2 ml-0 leading-tight text-gray-500 bg-white border border-gray-300  hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white";
};
...
function Pagination({ totalPage, currentPage, onPageChange }: PaginationProps) {
            ...
            <nav aria-label="Page navigation example">
                <ul className="inline-flex -space-x-px">
                    {Array.from({ length: totalPage }, (_, i) => (
                        <PageItem key={i} index={i + 1} current={currentPage} onPageChange={onPageChange} />
                    ))}
                </ul>
            </nav>
        );
    ...
    if (currentPage <= 3) {
        for (let i = 1; i <= 4; i++) {
            pages.push(<PageItem key={i} index={i} current={currentPage} onPageChange={onPageChange} />);
        }
